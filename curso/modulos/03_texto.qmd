...
- ¿En qué tipos de ambientes aparecen las especies *Rumex acetosa*, *Quercus ilex*, *Quercus suber* y *Acer campestre*?
:::
...
# Combinar archivos horizontalmente: `paste` y `join`

`paste` une archivos por **posición** (la línea 1 con la línea 1, la línea 2 con la línea 2...), sin mirar el contenido. 

```bash
~$ paste gene_info.csv gene_expression.csv
```
La opción `-d` permite modificar el carácter usado para unirlos. 

`join` es más útil y une dos archivos emparejando las líneas cuyo campo clave coincide. 
...
Usa el archivo [sitios_info.csv](../datasets/sitios_info.csv), con información de hábitat por sitio y *species_observations*.

El uso de `join` es el siguiente: 
...
## Sustituir caracteres: `tr`

`tr` (*translate*) sustituye caracteres por otros, uno a uno, en el texto que recibe por *stdin* — a diferencia de `sed`, no admite un nombre de archivo como argumento ni entiende patrones o columnas, solo traduce carácter por carácter:

```{.bash}
~$ cat gene_info.csv | tr '\t' ';'
gene;swissprot_id;description
pka;P12345;Protein kinase A catalytic subunit
...
```

::: {.callout-tip title=Cuestión}
- Convierte los tabuladores de `gene_info.csv` en `;` con `tr '\t' ';'`.
- Sobre ese resultado, convierte ahora los `;` en espacios con `tr ';' ' '`.
- Con ese segundo resultado, intenta extraer la columna `description` con `cut -d' ' -f3`.

¿Obtienes lo mismo que con `cut -d'\t' -f3` sobre el archivo original? ¿A qué se debe la diferencia?
:::
...
7. En `registro_campo_bruto.csv`, cuenta cuántas veces aparece cada localidad usando `cut`, `sort` y `uniq -c`.
...
14. Descomprime [muestras_fastq.zip](../datasets/muestras_fastq.zip) que contiene varios archivos `.fastq`. Une todos los archivos en uno solo. ¿Cuantas secuencias hay en total? (para descomprimir puedes hacerlo sobre el .zip con el botón derecho del ratón o bien ejecuta `unzip muestras_fastq.zip` en la terminal)
